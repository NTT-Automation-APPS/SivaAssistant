...
using SivaDigitalAssistant.Models;
using SivaDigitalAssistant.Services;
using System;
...
namespace SivaDigitalAssistant.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(activity.Text))
            {
                ...
                if (dispatchIntent == DispatchLuis.Intent.q_HRFaq)
                {
                    stepContext.SuppressCompletionMessage(true);

                    var knowledgebaseId = "HRFaq";
                    var qnaDialog = TryCreateQnADialog(knowledgebaseId, localizedServices);
                    if (qnaDialog != null)
                    {
                        Dialogs.Add(qnaDialog);
        ...
        private bool IsSkillIntent(DispatchLuis.Intent dispatchIntent)
        {
            if (dispatchIntent.ToString().Equals(DispatchLuis.Intent.l_General.ToString(), StringComparison.InvariantCultureIgnoreCase) ||
                ...
                dispatchIntent.ToString().Equals(DispatchLuis.Intent.q_HRFaq.ToString(), StringComparison.InvariantCultureIgnoreCase) ||
